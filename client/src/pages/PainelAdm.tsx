...
export default function PainelAdm() {
  ...
  const [abaAtiva, setAbaAtiva] = useState<'npcs' | 'pjs' | 'iniciativa' | 'fichas'>('npcs');
  ...
  const { fichas: fichasServidor, recarregar: recarregarFichas, deleteFicha: deleteFichaServidor, saveFicha: saveFichaServidor } = useFichaServerSync();
  ...
  const abas = [
    ...
    { id: 'fichas' as const, label: 'Fichas', icon: Star, count: fichasDisponiveis.length },
  ];
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="font-display text-2xl text-white tracking-wider">FICHAS DOS JOGADORES</h2>
                <p className="text-xs text-muted-foreground mt-1">
                  {fichasDisponiveis.length} ficha{fichasDisponiveis.length !== 1 ? 's' : ''} salva{fichasDisponiveis.length !== 1 ? 's' : ''} no servidor — visíveis de qualquer dispositivo
                </p>
              </div>
              <button
                onClick={() => recarregarFichas()}
                className="flex items-center gap-2 px-3 py-2 rounded-sm text-xs font-heading uppercase tracking-wider transition-colors"
                style={{ background: 'oklch(0.52 0.22 260 / 0.15)', color: 'oklch(0.75 0.15 230)', border: '1px solid oklch(0.52 0.22 260 / 0.3)' }}
              >
            ...
            {fichasDisponiveis.length === 0 ? (
              <div className="bl-card p-12 text-center">
                <Star className="w-12 h-12 mx-auto mb-4 opacity-20 text-primary" />
                <p className="text-muted-foreground font-heading uppercase tracking-wider text-sm">Nenhuma ficha salva ainda</p>
                <p className="text-xs text-muted-foreground mt-2">
                  As fichas aparecem aqui quando os jogadores clicam em "Salvar" na página de Criar Ficha.
                  Qualquer ficha salva de qualquer dispositivo aparecerá aqui automaticamente.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {fichasDisponiveis.map(ficha => {
                  ...
                  const classeNome = ficha.classId === 'personalizada'
                    ? (ficha.classePersonalizadaNome || 'Classe Personalizada')
                    : ficha.classId || 'Sem Classe';
                  const modoLabel = ficha.modoImperador === 'artilheiro' ? '⚽ Artilheiro'
                    : ficha.modoImperador === 'defensor' ? '🛡️ Defensor'
                    : null;

                  return (
                    <div key={ficha.id} className="bl-card overflow-hidden">
                      {/* Cabeçalho da ficha */}
                      <div className="flex items-center justify-between p-5">
                        <div className="flex items-center gap-4">
                          {ficha.imagemUrl ? (
                            <img src={ficha.imagemUrl} alt={ficha.nome} className="w-12 h-12 rounded-sm object-cover border border-white/10" />
                          ) : (
                            <div className="w-12 h-12 rounded-sm flex items-center justify-center" style={{ background: 'oklch(0.52 0.22 260 / 0.2)', border: '1px solid oklch(0.52 0.22 260 / 0.3)' }}>
                              <Crown className="w-6 h-6 text-primary" />
                            </div>
                          )}
                          <div>
                            <div className="flex items-center gap-2">
                              <h3 className="font-display text-xl text-white italic uppercase">{ficha.nome}</h3>
                              <span className="text-primary font-mono-stats text-sm">#{ficha.numero}</span>
                            </div>
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[10px] font-heading uppercase tracking-wider text-muted-foreground">{classeNome}</span>
                              {ficha.classId === 'imperador' && modoLabel && (
                                <span className="text-[9px] px-2 py-0.5 rounded-full font-heading uppercase tracking-wider"
                                  style={{
                                    background: ficha.modoImperador === 'artilheiro' ? 'oklch(0.65 0.22 55 / 0.2)' : 'oklch(0.55 0.22 260 / 0.2)',
                                    color: ficha.modoImperador === 'artilheiro' ? 'oklch(0.75 0.22 55)' : 'oklch(0.75 0.15 230)',
                                    border: `1px solid ${ficha.modoImperador === 'artilheiro' ? 'oklch(0.65 0.22 55 / 0.4)' : 'oklch(0.55 0.22 260 / 0.4)'}`
                                  }}
                                >{modoLabel}</span>
                              )}
                              {ficha.classId === 'imperador' && !modoLabel && (
                                <span className="text-[9px] px-2 py-0.5 rounded-full font-heading uppercase tracking-wider bg-white/5 border border-white/10 text-muted-foreground">Sem Modo</span>
                              )}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="hidden md:flex items-center gap-3 mr-4">
                            {Object.entries(ficha.atributos || {}).map(([attr, val]) => (
                              <div key={attr} className="text-center">
                                <div className="text-lg font-black text-white">{val as number}</div>
                                <div className="text-[9px] font-heading uppercase text-muted-foreground">{attr.slice(0, 3)}</div>
                              </div>
                            ))}
                          </div>
                          <button
                            onClick={() => setModalEditarFicha({ aberto: true, ficha })}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs font-heading uppercase tracking-wider transition-colors"
                            style={{ background: 'oklch(0.52 0.22 260 / 0.15)', color: 'oklch(0.75 0.15 230)', border: '1px solid oklch(0.52 0.22 260 / 0.3)' }}
                          >
                          ...
                          <button
                            onClick={() => {
                              if (confirm(`Deletar a ficha de "${ficha.nome}"? Esta ação não pode ser desfeita.`)) {
                                deleteFichaServidor(ficha.id);
                                toast.success(`Ficha de ${ficha.nome} removida.`);
                              }
                            }}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs font-heading uppercase tracking-wider transition-colors"
                            style={{ background: 'oklch(0.4 0.1 0 / 0.15)', color: 'oklch(0.7 0.1 0)', border: '1px solid oklch(0.4 0.1 0 / 0.3)' }}
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                          <button
                            onClick={() => setFichaExpandida(isExpanded ? null : ficha.id)}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm text-xs font-heading uppercase tracking-wider transition-colors bg-white/5 border border-white/10 text-muted-foreground hover:text-white"
                          >
                      ...
                      {/* Detalhes expandidos */}
                      {isExpanded && (
                        <div className="border-t px-5 pb-5 pt-4 space-y-6" style={{ borderColor: 'oklch(0.22 0.03 260)' }}>
                          {/* Perícias */}
                          <div>
                            <p className="text-[10px] font-heading uppercase tracking-widest text-primary mb-3">PERÍCIAS</p>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                              {Object.entries(ficha.pericias || {}).filter(([, v]) => (v as number) > 0).map(([skill, val]) => (
                                <div key={skill} className="flex items-center justify-between p-2 rounded-sm bg-white/5 border border-white/5">
                                  <span className="text-[10px] text-muted-foreground truncate">{skill}</span>
                                  <span className="text-sm font-mono-stats text-primary ml-2">+{val as number}</span>
                                </div>
                              ))}
                              {Object.values(ficha.pericias || {}).every(v => (v as number) === 0) && (
                                <p className="text-xs text-muted-foreground col-span-4">Nenhuma perícia distribuída.</p>
                              )}
                            </div>
                          </div>

                          {/* Modo Imperador se aplicar */}
                          {ficha.classId === 'imperador' && (
                            <div className="p-4 rounded-sm" style={{ background: 'oklch(0.52 0.22 260 / 0.08)', border: '1px solid oklch(0.52 0.22 260 / 0.2)' }}>
                              <p className="text-[10px] font-heading uppercase tracking-widest text-primary mb-2">META VISÃO</p>
                              {ficha.modoImperador ? (
                                <p className="text-xs text-white">
                                  Modo <span className="font-bold" style={{ color: ficha.modoImperador === 'artilheiro' ? 'oklch(0.75 0.22 55)' : 'oklch(0.75 0.15 230)' }}>
                                    {ficha.modoImperador === 'artilheiro' ? 'Artilheiro' : 'Defensor'}
                                  </span> ativo.
                                  {ficha.modoImperador === 'artilheiro'
                                    ? ' Chute, Drible e Presença com +3. Demais perícias com +1.'
                                    : ' Corpo a Corpo, Reflexos e Roubo De Bola com +3. Demais perícias com +1.'
                                  }
                                </p>
                              ) : (
                                <p className="text-xs text-muted-foreground">Nenhum modo selecionado. Todas as perícias com +1 base.</p>
                              )}
                            </div>
                          )}
                          ...
                          {ficha.armaNome && (
                            <div>
                              <p className="text-[10px] font-heading uppercase tracking-widest text-primary mb-2">ARMA BLUE LOCK</p>
                              <p className="text-sm font-bold text-white">{ficha.armaNome}</p>
                              {ficha.armaDescricao && <p className="text-xs text-muted-foreground mt-1">{ficha.armaDescricao}</p>}
                              {ficha.armaBonus && <p className="text-xs text-primary mt-1">Bônus: {ficha.armaBonus}</p>}
                            </div>
                          )}
                          ...
                          {ficha.notas && (
                            <div>
                              <p className="text-[10px] font-heading uppercase tracking-widest text-primary mb-2">NOTAS</p>
                              <p className="text-xs text-muted-foreground leading-relaxed">{ficha.notas}</p>
                            </div>
                          )}

                          <p className="text-[9px] text-muted-foreground/50">
                            Salvo em: {ficha.atualizadoEm ? new Date(ficha.atualizadoEm).toLocaleString('pt-BR') : 'desconhecido'}
                          </p>
                        </div>
                      )}
                    </div>
